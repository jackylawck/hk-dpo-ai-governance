...
# ==========================================
# 4. 分頁一：智能情境問答 (Scenario Q&A) - 新增聊天介面
# ==========================================
with tabs[0]:
    ...
    if "messages" not in st.session_state:
        ...
        # 給一個預設的歡迎訊息
        welcome_msg = "您好！我是您的 AIGP 企業合規助手。請問今天有什麼 AI 導入的場景或風險挑戰需要探討？" if is_zh else "Hello! I am your AIGP compliance assistant. What AI deployment scenario would you like to discuss today?"
    ...
    # 處理使用者輸入
    if prompt := st.chat_input("請輸入企業情境... (Type your scenario here...)" if is_zh else "Type your scenario here..."):
        # 顯示使用者的問題
        ...
        with st.chat_message("assistant"):
            if "iPad" in prompt or "私自" in prompt or "影子" in prompt or "客戶名單" in prompt or "shadow" in prompt.lower():
...
2. **紀律處分與建立白名單 (Enforcement & Whitelisting)：** 依據政策對違規行為進行處置，同時 HR 應協同 IT 部門，儘速提供已通過安全審查的內部企業版 AI 工具（第 1 項），消除員工繞過管控的動機。""" if is_zh else """🚨 **Critical Breach Detected: "Shadow AI" and Potential "Data Breach"!**
...
**Top 2 Governance Actions Recommended:**
1. **Trigger Incident Response:** Assess the severity of the PII leak and contain the exposure immediately.
2. **Enforcement & Whitelisting:** Enforce policy consequences and quickly provide employees with a secure, approved enterprise AI alternative to eliminate the need for Shadow AI."""
            else:
                response = """💡 **收到您的情境描述。**
                
這是一個典型的 AI 管治議題。從 AIGP 框架（Domain III & IV）出發，我們建議您切換至上方的 **「📝 14項法定內部政策」** 標籤頁，逐一檢核您的企業是否已經明確定義了「獲准許的用途」、「輸入資訊的種類」以及「人類最終問責」的機制。

*(提示：您可以試著輸入類似「市場部主管私自用 iPad 上傳客戶名單給免費 AI」的情境來測試系統。)*""" if is_zh else """💡 **Scenario Logged.**
                
From an AIGP governance perspective (Domains III & IV), we recommend navigating to the **"📝 14-Point Checklist"** tab above to verify if your organisation has clearly defined "Permissible uses" and "Human-in-the-loop" accountability.
...
# ==========================================
# 5. 分頁二：風險與行業管治 (整合先前的 Risk Triage 與 Industry)
# ==========================================
with tabs[1]:
    st.subheader("第一步：風險分類系統 (Risk Classification System)" if is_zh else "Step 1: Risk Classification System")
    # (此處保留原有的風險定級代碼，為了簡潔，使用簡化版)
    st.info("📌 依據指引第九至十頁：高風險系統必須進行合規性評估；有限風險系統須提供用戶選擇退出的機制。" if is_zh else "📌 High Risk systems require conformity assessments; Limited Risk requires opt-out mechanisms.")
...
with tabs[2]:
    ...
    st.markdown("---")
    st.markdown(f"### 📊 機構內部政策指引完備度: **{readiness_score:.0f}%** ({passed_counts}/14)")
